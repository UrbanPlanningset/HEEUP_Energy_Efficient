...
class low_Agent:
    ...
    def act(self, states, current_height, action_space_mask=None):
        with torch.no_grad():
            # 将 states 转换为 Tensor
            ...
            probs = log_probs.exp().squeeze().cpu().numpy()  # 确保是一维数组

            height_changes = np.array([-2, -1, 0, 1, 2])

            expected_heights = current_height + height_changes

            action_space_mask = (expected_heights > 0)

            masked_probs = np.zeros_like(probs)

            masked_probs[action_space_mask] = probs[action_space_mask]

            if masked_probs.sum() > 0:
                masked_probs /= masked_probs.sum()
            else:
                masked_probs = np.ones_like(probs) / len(probs)
            action = np.random.choice(a=np.arange(self.action_size), p=masked_probs)
            return masked_probs, action
    ...
    def learn(self, state_values, log_probs, rewards, discounted_rewards, dones):
        ...
        Critic_Loss = 0.5 * (state_values - G).pow(2).mean()
